...
import { X, Phone, Edit2, Save, Plus, User, Clock, MessageCircle } from 'lucide-react';
...
function useContactNotes(contactId?: string) {
  ...
  return { notes: query.data ?? [], isLoading: query.isLoading, addNote };
}
...
export function WhatsAppContactPanel({
  ...
  const { notes, addNote } = useContactNotes(contact?.id);
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-4">
          ...
          <div>
            ...
            <div className="space-y-2">
              {notes.map(note => (
                <div key={note.id} className="p-2 rounded bg-muted/50 text-xs">
